            ...
            <div class="relative bg-slate-800 border border-slate-700 rounded-2xl shadow-2xl p-8 max-w-sm w-full mx-4">
                <div class="flex flex-col items-center text-center gap-4">
                    ...
                    <div class="flex gap-3 w-full mt-2">
                        <button @click="$emit('cancel')" class="flex-1 px-4 py-2 rounded-xl bg-slate-700 text-white hover:bg-slate-600 transition-colors">Cancelar</button>
                        <button @click="$emit('confirm')" class="flex-1 px-4 py-2 rounded-xl bg-red-500 text-white hover:bg-red-400 transition-colors font-semibold">Borrar</button>
                    </div>
                </div>
            </div>
        </div>
